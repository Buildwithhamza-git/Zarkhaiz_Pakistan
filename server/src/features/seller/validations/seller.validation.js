...
/**
 * Schema for updating an existing seller profile.
 *
 * Business rules:
 * - CNIC, business type and status CANNOT be changed here (identity data).
 * - Payment details follow the same rules as registration:
 *   at least one method, account title always required, bank needs both
 *   bankName + iban, wallet numbers must be valid PK mobile numbers.
 */
const UpdateSellerProfileSchema = z
  .object({
    storeName: z
